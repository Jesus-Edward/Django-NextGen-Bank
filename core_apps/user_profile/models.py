...
class Profile(TimestampedModel):
    ...
    identification_means = models.CharField(
        choices=IndentificationMeans.choices,
        default=IndentificationMeans.NATIONAL_ID,
        max_length=50,
    ...
    date_of_birth = models.DateField(
        verbose_name=_("Date of Birth"), default=settings.DEFAULT_BIRTH_DATE
    )
    country_of_birth = models.CharField(
        verbose_name=_("Country of Birth"), default=settings.DEFAULT_COUNTRY
    )
    place_of_birth = models.CharField(
        verbose_name=_("Place of Birth"), default="Unknown", max_length=50
    )
    id_issued_date = models.DateField(
        _("ID or Passport Issued Date"), default=settings.DEFAULT_DATE
    )
    id_expiry_date = models.DateField(
        _("ID or Passport Expiry Date"), default=settings.DEFAULT_EXPIRY_DATE
    )
    id_number = models.CharField(
        _("ID or Passport Number"), default=settings.DEFAULT_EXPIRY_DATE, max_length=50
    )
    ...
    country = CountryField(
        _("Country"), default=settings.DEFAULT_COUNTRY, max_length=100
    )
    employer_name = models.CharField(
        _("Employer Name"), max_length=50, blank=True, null=True
    )
    annual_income = models.DecimalField(
        _("Annual Income"), default=0.0, decimal_places=2, max_digits=12
    )
    date_of_employment = models.DateField(
        _("Date of Employment"), blank=True, null=True
    )
    employer_address = models.CharField(
        _("Employer Address"), blank=True, null=True, max_length=150
    )
    employer_city = models.CharField(
        _("Employer City"), blank=True, null=True, max_length=100
    )
        ...
        if self.id_issued_date and self.id_expiry_date:
            if self.id_expiry_date <= self.id_issued_date:
                raise ValidationError(
                    _("ID expiry date must come after the issued date.")
                )
