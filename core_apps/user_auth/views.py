...
from .emails import send_email_otp
...
class CustomTokenCreateView(TokenCreateView):
    def _action(self, serializer):
        ...
        if user.is_locked_out:
            return Response(
                "error:"
                f"Account is locked due to multiple login attempts, Please try again after {settings.LOGOUT_DURATION.total_seconds() / 60} minutes",
        ...
        logger.info("OTP:" f"OTP sent successfully to user email: {user.email}")
    ...
    def post(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        try:
            ...
            if user:
                ...
                logger.error(
                    f"Failed login attempts: {failed_attempts} for the user {email}"
                )

                if failed_attempts >= settings.LOGIN_ATTEMPTS:
                    return Response(
                        {
                            "error": f"You've exceeded the maximum login attempts, your account has been locked for {settings.LOGOUT_DURATION.total_seconds() / 60} minutes. An email has been sent to you with further instructions."
                        },
                        status = status.HTTP_403_FORBIDDEN,
                    )
            else:
                logger.error("error:" f"Failed login attempt for non-existing user {email}")

            return Response(
                {
                    "error": f"Your login credentials do not match our records"
                },
    ...
    def post(self, request: Request, *args: Any, **kwargs: Any):
        ...
        if refresh_res.status_code == status.HTTP_200_OK:
            ...
            if access_token and refresh_token:
                ...
                refresh_res.data["message"] = "Access or refresh token is not found in refresh response data."
                logger.error("Access or refresh token is not found in refresh response data.")
...
class OTPVerifyView(APIView):
    ...
    def post(self, request):
        ...
        if user.is_locked_out:
            return Response(
                {"error": f"Account is locked due to multiple failed login attempts. Please try again after {settings.LOGOUT_DURATION.total_seconds() / 60} minutes"},
                status=status.HTTP_403_FORBIDDEN
            )
        ...
        response =  Response(
            {"success": "Login successfully, please complete your profile information so that an account can be created for you."},
            status=status.HTTP_200_OK
        )
