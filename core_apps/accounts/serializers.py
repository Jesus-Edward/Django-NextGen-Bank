...
class AccountVerificationSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        ready_only_fields = ["fully_activated"]
    ...
    def validate(self, data: dict) -> dict:
        ...
        if kyc_verified:
            ...
            if not verification_notes:
                raise serializers.ValidationError(_("Verification notes is required when verifying an account."))

            if kyc_submitted and not all(["kyc_verified", "verification_date", "verification_notes"]):
                raise serializers.ValidationError(_("All Verification fields (KYC Verified, verification date, and notes are must be provided when KYC is submitted."))
...
class DepositSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance: BankAccount):
...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance: Transaction) -> str:
        ...
        representation["receiver_account"] = (
            instance.receiver_account.account_number if instance.receiver_account else None
        )
        ...
        try:
            if transaction_type == Transaction.TransactionType.WITHDRAWAL:
                ...
                if account.account_balance < amount:
                    raise serializers.ValidationError(_("Insufficient funds for withdrawal"))
                ...
                if sender_account == receiver_account:
                    raise serializers.ValidationError(_("Sender and receiver account must be different."))

                if sender_account.currency != receiver_account.currency:
                    raise serializers.ValidationError(_("Transfers are only allowed between the same currency"))

                if sender_account.account_balance < 0:
                    raise serializers.ValidationError(_("Insufficient funds for transfer."))
