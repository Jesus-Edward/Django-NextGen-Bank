...
def generate_transaction_pdf(user_id, start_date, end_date, account_number=None):
    try:
        ...
        if account_number:
            account = BankAccount.objects.get(user=user, account_number=account_number)
            transactions = transactions.filter(
                Q(sender_account=account) | Q(receiver_account=account),
            ).order_by("-created_at")

        buffer = BytesIO() # An in-memory buffer to store the pdf data
        ...
        data = [ # represents the table data
        ...
        for transaction in transactions:
            data.append(
                [
                    transaction.created_at.strftime("%Y-%m-%d %H:%I:%S"),
...
def detect_suspicious_activities():
    ...
    for account in accounts:
        ...
        if suspicious_activities:
            ...
            if num_activities > 0:
                return (
                    f"Suspicious activity check completed. {num_activities} suspicious activities detected and reported"
                )
            else:
                return (
                    f"Suspicious activity check completed. Activities detected but alert email failed to send"
                )
